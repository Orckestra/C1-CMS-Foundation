...
<xsl:stylesheet version="1.0" exclude-result-prefixes="x" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/1999/xhtml" xmlns:x="http://www.w3.org/1999/xhtml">
	...
	<xsl:variable name="uriWithPort" select="concat($requestscheme,'://',$requesthostname,':',$requestport,'/')" />
	<xsl:variable name="uriNoPort" select="concat($requestscheme,'://',$requesthostname,'/')" />
	
	<!-- these will be stripped from innerHTML input in order to allow relative URLs -->
	<xsl:variable name="uriEditor">Composite/content/misc/editors/wysiwygeditor/</xsl:variable>
	<xsl:variable name="uriEditor1" select="concat('/',$uriEditor)"/>
