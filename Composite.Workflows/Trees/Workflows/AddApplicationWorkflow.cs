...
namespace Composite.Workflows.Trees.Workflows
{
    ...
    public sealed partial class AddApplicationWorkflow : Composite.Workflow.Activities.FormsWorkflow
    {
        ...
        private void initializeCodeActivity_UpdateBindings_ExecuteCode(object sender, EventArgs e)
        {
            ...
            foreach (Tree tree in TreeFacade.AllTrees)
            {
                ...
                if (tree.HasAttachmentPoints(this.EntityToken) == true) continue;
        ...
        {
            this.ShowMessage(DialogType.Message, "${Composite.Trees, AddApplication.NoTrees.Title}", "${Composite.Trees, AddApplication.NoTrees.Message}");
        }
    }
}
