	...
	tinymce.create("tinymce.plugins.CompositeComponentPlugin", {
		...
		_resolveParametersAndInsertHtml: function (componentDocument) {
			...
			if (ffunction != null) {
				...
				FunctionService.TryExecute(ffunction.xml || ffunction.outerHTML, function (functionExecutionResult) {
					if (functionExecutionResult.FunctionCallExecutable) {
						self._insertImagTagAndContinue(ffunction, ffunction.xml || ffunction.outerHTML, componentDocument);
					} else {
				...
				self._insertResult(componentDocument.lastChild.xml || componentDocument.lastChild.outerHTML);
			}
		},
		...
		_launchUpdateDialog: function (ffunction, dialogHandler) {
			...
			EditorBinding.invokeFunctionEditorDialog(ffunction.xml || ffunction.outerHTML, dialogHandler, "Composite.Core.Xml.XhtmlDocument, Composite", this.editor.theme.editorBinding)
		},
