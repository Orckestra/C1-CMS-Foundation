    ...
    public class Generic : HtmlGenericControl 
    {
        ...
        protected class PostBackEventHandler : IPostBackEventHandler
        {
            ...
            public void RaisePostBackEvent(string eventArgument)
            {
                ...
                {
                    Logging.LoggingService.LogError(typeof(Generic).FullName, "Failed to find parent control, that appropriate for event handling".FormatWith(methodName));
                    return;
                }
                ...
                if(methodInfo == null)
                {
                    Logging.LoggingService.LogError(typeof(Generic).FullName, "Failed to find method '{0}'".FormatWith(methodName));
                    return;
                }
